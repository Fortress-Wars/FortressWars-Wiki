...
Players now have an additional `15` seconds to prepare before the game starts.
...
- Decreased bricks knockback from **4** to **3**.
- Increased bricks damage from **5** to **6**.

Gunner

- Increase machine gun bullet damage from **2** to **2.5**.
- Decrease overheat percentage per shot from **4** to **3.5**.
...
- Increased max stomp damage from **20** to **30**.
- Increased stomp radius from **3** to **3.5**.
...
- Increased staff beam range from **10** meters to **15** meters.

Musketeer

- The player can no longer scope-in with their rifle while in "Burst Mode".
- The rifle now stops firing if player stops holding the rifle item.
- Increase sniper rifle damage from **8** to **10**.
...
- `/staffchat off`
  - Turn off staff chat.
- `staffchat message`
  - Send a single message to staff chat.
